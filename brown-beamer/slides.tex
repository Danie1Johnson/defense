...
$$E\left[\tau^{[F]}\right] &= \left[\left(\diag\left(\mathbbm{1}_{[F]}\right) - \diag\left(\mathbbm{1}_{[F]^c}\right)Q\right)\right]^{-1} \mathbbm{1}_{[F]^c}$$
  \centering
    \scalebox{0.25}{
      \begin{figure}
        \centering
        \includegraphics{octahedron_tau_one.pdf}
       \end{figure}
      }
\end{frame}
...
\begin{itemize}
\item Fix translations by imposing additional constraints on center of mass.
\item Find bases $W \in \mathbbm{R}^{n\times 3}$ that correspond to rotation directions.
\item Use $\tilde{A} = [C^T(z) W \tilde{B}]$ where $\tilde{B} \in \mathbbm{R}^{n\times (n-m-3)}
...
\end{frame}
% -----------------------------------------------------------------------------
\begin{frame}{Markov Process on Octahedron CCS}
  \centering
    \scalebox{0.15}{
      \begin{figure}
        \centering
